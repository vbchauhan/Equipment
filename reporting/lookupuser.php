...
?>
<BR>
<H1>TYPE IN USER DETAIL BELOW</H1>
<?php
if (@$message) 
...
?>
	...
	<tr>
		...
		<form name="form1scan" method="post" action="">
			...
			<p align="center">TYPE IN NAME OR EMAIL OR LOGINID HERE: <input type="text" name="userinputdata" size="50" maxlength="50" value="">
...
if ($action=='list')
	...
	$ListQuery = "SELECT 
					...
					FROM
						 loansystemlogs
					WHERE (((lower(loantologinid )='".strtolower(@$_POST["userinputdata"])."' ) or 
							(lower(loantoemail )='".strtolower(@$_POST["userinputdata"])."' ) or 
							(lower(loantoname ) LIKE '%".strtolower(@$_POST["userinputdata"])."%' )) and 
							(action='checkinitem'))
...
<BR>
		...
		<TR>
			...
			<TD WIDTH="250" class="tableheading" ALIGN="center"><strong>ItemBarcode</strong></TD>
			<TD WIDTH="50" class="tableheading" ALIGN="center"><strong>LoanToName</strong></TD>
			<TD WIDTH="50" class="tableheading" NOWRAP ALIGN="center"><strong>LoanGroup</strong></TD>					
		...
		<tr>
			<td width="250" CLASS="tablebody" align="center" colspan="6">Error: Unable to find a loan history for <?php echo $_POST["userinputdata"];?> in this system!</td>
		</tr>
        </table>
        </div>
		<?php
...
<BR><H3 align="center"><B><?php echo strtoupper(@$_POST["userinputdata"]);?> Loan History</B></H3><?php
	...
	?>	
		...
		<TR>
			...
			<TD class="tablebody" NOWRAP ALIGN="left"><?php echo $ListQueryRow["loanitembarcode"];?></TD>
			<TD class="tablebody" NOWRAP ALIGN="left"><?php echo $ListQueryRow["loantoname"];?></TD>
			<TD class="tablebody" NOWRAP ALIGN="center"><?php echo $ListQueryRow["loangroup"];?></TD>					
        ...
        <form name="form1scan" method="post" action="download.php" target="_blank">
		    <input type="hidden" name="action" value="UserHistoryCSVl">
		    <input type="hidden" name="lgroup" value="<?php echo $lgroup?>">
