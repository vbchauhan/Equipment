...
//if (strlen($action)<=1) $action='loangroupselect';

if (($action=='listgroupset') or (strlen($lgroup)>=2))
	{
	$_SESSION["LOANSYSTEM_GROUP"]=$lgroup;
	} //END if ($action=='listgroupupdate')

top();



if (($action=='loangroupselect') or (strlen($_SESSION["LOANSYSTEM_GROUP"])<=1))
	{
...
<table width="300" border="0" align="center"><?
		...
		<tr>
			...
			<INPUT TYPE="hidden" NAME="action" VALUE="listgroupset">
            ...
            <?php echo $Rows['loangroupdescription']?><BR><BR>
            </td>
		</tr>
		<?php
...
<table width="300" border="0" align="center">
	...
	} //END if (($action=='loangroupselect') or (strlen($_SESSION["LOANSYSTEM_GROUP"])<=1))


if ((strlen($_SESSION["LOANSYSTEM_GROUP"])>=2) and ($action<>'loangroupselect'))
	{
	$GroupPublicViewQuery="select viewitemstatuswithoutlogin FROM loangroups WHERE loangroup='".$_SESSION["LOANSYSTEM_GROUP"]."'";
	$GroupPublicViewResult = @mysql_query($GroupPublicViewQuery);
	$GroupPublicViewRows = @mysql_fetch_array($GroupPublicViewResult);
	if ($GroupPublicViewRows['viewitemstatuswithoutlogin']=='1')
		$GroupPublicView='1';
	else
		$GroupPublicView='0';
	if (@$_SESSION["AUTH_USER"]==true) $GroupPublicView='1';
	} //END if ((strlen($_SESSION["LOANSYSTEM_GROUP"])>=2) and ($action<>'loangroupselect'))


if ((strlen($_SESSION["LOANSYSTEM_GROUP"])>=2) and ($action<>'loangroupselect') and ($GroupPublicView=='1'))
		{
		...
		if ($rowqueryloangroup["verbosedisplay"]=='1')
			...
			$colspanvalue=3;			
		?>
		<H1 align="left">LOAN ITEMS STATUS FOR THE <?php echo $_SESSION["LOANSYSTEM_GROUP"];?> GROUP</H1>
		...
		<table align="center" width="90%" border="0" class="tablebox">
   			<TR> 
				...
				if ($GroupNums>=2)
					{
					?>
           		<FORM NAME="form1Select" METHOD="post" ACTION="" ENCTYPE="multipart/form-data">
					<INPUT TYPE="hidden" NAME="action" VALUE="loangroupselect">
					<INPUT TYPE="submit" name="Submit" value="Change Loan Group">                
    			</FORM>
                	<?php
					}
				else
                ...
                <TD align="right" colspan="<?=$colspanvalue-1?>">
			...
			if ($rowqueryloangroup["verbosedisplay"]=='1')
				...
				foreach($SortArray as &$ma)
					...
					print '<a href="/'.strtolower($_SESSION["SystemNameStr"]).'/view/loanitemdetail.php?action=viewitem&uid='.$ma[15].'"'.$TextDecoration.' target="_self">';
...
<table width="600" border="0" align="center">
        ...
        <tr>
			<td align="center" CLASS="tablebody">
            <em>No loan items avaliable in this loan group!</em><BR>
            </td>
		</tr>
</table>
</div>				
